...
function MDL_SCORM_SCOES_TRACK_CO(){
    ...
    cron.schedule('0 7,17 * * *',() => {
     conn.execute({
        sqlText:`SELECT * FROM DIDACTIK.MOODLE_BI.MDL_SCORM_SCOES_TRACK_CO b WHERE DATE(TO_TIMESTAMP_NTZ(b.timemodified)) BETWEEN DATEADD(day, -8, DATE(GETDATE())) AND DATE(GETDATE()) AND b.scormid = 4670
...
function MDL_USER_CO(){
  cron.schedule('0 6,16 * * *',() => {
    conn.execute({
       sqlText:`select * from DIDACTIK.MOODLE_BI.MDL_USER_CO b WHERE DATE(TO_TIMESTAMP_NTZ(b.timemodified)) BETWEEN DATEADD(day, -15, DATE(GETDATE())) AND DATE(GETDATE())`,
       complete: (err, stmt, rows) => {
         if(err){
           console.log(err.message)
         }else{
...
function MDL_GRADE_GRADES_CO(){
  cron.schedule('0 19 * * *',() => { 
    conn.execute({
       sqlText:`SELECT * FROM DIDACTIK.MOODLE_BI.MDL_GRADE_GRADES_CO b WHERE DATE(TO_TIMESTAMP_NTZ(b.timecreated)) BETWEEN DATEADD(day, -90, DATE(GETDATE())) AND DATE(GETDATE()) OR DATE(TO_TIMESTAMP_NTZ(b.timemodified)) BETWEEN DATEADD(day, -90, DATE(GETDATE())) AND DATE(GETDATE())`,
       complete: (err, stmt, rows) => {
         if(err){
           console.log(err.message)
         }else{
